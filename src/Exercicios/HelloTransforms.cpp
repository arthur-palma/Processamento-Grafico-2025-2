...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = translate(model, vec3(400.0f, 300.0f, 0.0f)); //Translada para o centro da tela
		model = scale(model, vec3(400.0f, 400.0f, 1.0f)); //Escala o triângulo
		model = rotate(model, radians(-90.0f), vec3(0.0f, 0.0f, 1.0f)); //Rotaciona o triângulo
		//Enviando a matriz de projeção para o vertex shader
		...
		model = translate(model, vec3(200.0f, 300.0f, 0.0f)); //Translada para o centro da tela
		model = scale(model, vec3(400.0f, 400.0f, 1.0f)); //Escala o triângulo
		model = rotate(model, radians(-90.0f), vec3(0.0f, 0.0f, 1.0f)); //Rotaciona o triângulo
		//Enviando a matriz de projeção para o vertex shader
		glUniformMatrix4fv(glGetUniformLocation(shaderID, "model"), 1, GL_FALSE, value_ptr(model));
