	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				sprintf(tmp, "Ex6b -- Rossana\tFPS %.2lf", fps);
...
int createCircle(int nPoints, float radius)
{
	...
	radius = 0.5;
	float angle = 0.0;
	float angleStep = 2 * Pi / (float)nPoints;

	vertices.push_back(0.0); // Centro x
	vertices.push_back(0.0); // Centro y
	vertices.push_back(0.0); // Centro z
